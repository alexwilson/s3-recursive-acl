...
func main() {
	...
	var grants []*s3.Grant
	var success, matched, counter int64
	...
	if err != nil {
		log.Panicf("Unable to compile regex '%s', %v\n", includeRegex, err)
	}
	...
	if !dryrun {
		log.Printf("Updated permissions on %d objects out of %d (regex matched : %d)\n", success, counter, matched)
	} else {
		log.Printf("DRY RUN : Updated permissions on %d objects out of %d (regex matched : %d)\n", success, counter, matched)
	}
}
